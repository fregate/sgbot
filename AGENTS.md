...
- **Never commit real credentials.** The committed versions of `sgbot/bot-func_test.go` and `gogbot/test.go` use placeholders on purpose. Real keys/cookies may exist in the *working tree* for local debugging — they must stay uncommitted (check `git diff` before committing).
- **Tests here are live, side-effecting E2E tests, not unit tests.** `TestBotFunc` (sgbot) hits the real Steam API and real SteamGifts pages through the paid ZenRows proxy and *actually enters giveaways with a real account*. Even on a host with Go installed, never run the test suite "just to check" — it consumes ZenRows quota and acts on a live account.
...
|---|---|---|---|
| `sgbot/` | `sgbot.mem4.me/sgbot` | 1.23.1 | Main bot: 3 cloud-function entry points in one `package main` |
...
| `README.md` | — | — | Human-facing deploy guide (still accurate for the Yandex flow) |
| `tools/` | — | — | Python helper scripts, not part of any Go module or deployment |
...
- `thebot.go` — core logic. `TheBot` struct; Steam API calls (`fetchWishlist`, `fetchFollowedList` via `api.steampowered.com`); SteamGifts page fetch + parse through the **ZenRows** client (`scraperapi`; `JSRender: true`, `WaitForSelector: "body"` — this is how the bot bypasses Cloudflare); giveaway parsing with goquery; token extraction; `/ajax.php` entry POST.
- `bot-func.go` — **cloud function entry `RunSGBOTFunc(ctx) (*Response, error)`**. Reads env, loads cookies/games from YDB, runs the bot, writes errors + digest to YDB.
- `bot-init-func.go` — **entry `RunInitBotDB`**: creates YDB tables `games(id:uint64 PK, name:string)`, `cookies(name:string PK, value, domain, path)`, `digest(message:UTF8 PK)`.
...
- `yandex.sgbot-func.deploy.sh` → `sgbot/`: `bot-func.go thebot.go go.mod func-response.go sorter.go`
...
- Env vars: `YDB_DATABASE` (all functions), `STEAM_PROFILE`, `STEAM_API_KEY`, `ZENROW_KEY` (bot), `MAILER_SMTP`, `MAILER_PORT`, `MAILER_AUTH_NAME`, `MAILER_AUTH_PWD`, `MAILER_SUBJECT`, `MAILER_RECIPIENT` (digest).
...
- `.github/workflows/go.yml` — CI from the old local-daemon architecture: it runs `go get` / `go generate`, hand-fabricates a `pathes.go`, then `go build ./...` on `sgbot/`, which has no `main()` — the workflow is broken as-is. Don't patch it into the old shape; if CI is wanted for the current layout, rewrite it per module (build only — the tests are live E2E and must never run in CI).
...
- Git: the repo is currently in a **detached HEAD** state at `origin/master`. Make changes on a named branch, not in detached HEAD. `checkdp/` is untracked on purpose (scratch with secrets).
